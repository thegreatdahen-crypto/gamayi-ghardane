// ... (هەموو کۆدی سەرەتای script.js وەک خۆی تا دەگەیت بە Game variables) ...

// Game variables
let player, obstacles, score, gameLoop, groundHeight, cameraX, door; // 'door' زیادکرا
let gameActive = false;
let currentLevel = 1;

// ... (levelConfig و setCanvasSize وەک خۆیان) ...
...
function startLevel(level) {
    ...
    // دروستکردنی بەربەستەکان
    for (let i = 800; i < config.length - 200; i += Math.random() * 400 + 300) { // دڵنیادەبینەوە کە هیچ بەربەستێک زۆر نزیک دەرگاکە نییە
        createObstacle(i);
    }

    // --- **بەشی نوێ: دروستکردنی دەرگا لە کۆتایی ئاست** ---
    door = {
        x: config.length,
        y: groundHeight - 100, // با دەرگاکە بەرز بێت
        width: 80,
        height: 100
    };
...
function update() {
    ...
    // --- وێنەکێشانی هەموو شتەکان ---
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    ctx.fillStyle = '#e67e22'; ctx.fillRect(0, groundHeight, canvas.width, 50);
    
    obstacles.forEach(drawObstacle);
    drawDoor(); // وێنەکێشانی دەرگا
    drawPlayer();
...
function jump() { /* ... هەمان کۆدی پێشوو ... */ }

// ... (هەموو Event Listeners وەک خۆی) ...
// ... (Initial Call وەک خۆی) ...
...
function startLevel(level) {
    currentLevel = level; const config = levelConfig[level]; if (!config) { gameOver(true); return; }
    setCanvasSize();
    player = { x: 60, y: groundHeight - 60, width: 40, height: 60, velocityY: 0, isJumping: true };
